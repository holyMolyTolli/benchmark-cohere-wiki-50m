...
def compute_benchmark_metrics(file_list):
    ...
    for filepath in file_list:
        ...
        entry = {"filepath": filepath, "start_time": start_time, "end_time": end_time}
        for key, pattern in PARAM_PATTERNS.items():
            match = re.search(pattern, filename)
            if match:
                entry[key] = match.group(1)
            else:
                entry[key] = DEFAULT_VALUES[key]
        ...
        # --- 4. Structure Output ---
        entry.update(**{"search_" + k + "_ms": v * 1000 for k, v in search_stats.items()})
        entry.update(**{"server_" + k + "_ms": v * 1000 for k, v in server_stats.items()})
...
def find_experiment_for_timestamp(timestamp, benchmark_df, add_seconds=0):
    ...
    files = benchmark_df[mask]["filepath"].values.copy()
    if len(files) == 1:
        return files[0]
    elif len(files) > 1:
